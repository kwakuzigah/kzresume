...
import com.fordemobile.billing.Security.PurchaseState;
import com.fordemobile.livepaintings.Consts;
...
public class ResponseHandler {
    ...
    public static void purchaseResponse (
        ...
        // first.
    	//TODO notification depending on the purchaseState
//        Log.d(TAG, "purchaseResponse");
        ...
        new Thread(new Runnable() {
            public void run() {
		    	...
		    	if (productId.equals(Consts.PACK_001)) {
					SharedPreferences prefs = context.getSharedPreferences(Consts.SHARED_PREFS_NAME, Context.MODE_PRIVATE);
					SharedPreferences.Editor edit = prefs.edit();
					edit.putBoolean(Consts.PACK_001, true);
					edit.commit();
		    	}
     ...
     * of the request
     */
    public static void responseCodeReceived(RequestPurchase request,
            Result result) {
        if (sPurchaseObserver != null) {
            sPurchaseObserver.onRequestPurchaseResponse(request, result);
        }
    }
     ...
     *     of the request
     */
    public static void responseCodeReceived(RestoreTransactions request,
    		Result result) {
        if (sPurchaseObserver != null) {
            sPurchaseObserver.onRestoreTransactionsResponse(request, result);
        }
    }
}
